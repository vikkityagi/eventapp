...
class PedestrainReport extends StatefulWidget {
  const PedestrainReport({Key? key}) : super(key: key);
  @override
  State<PedestrainReport> createState() => _MyAppState();
}

class _MyAppState extends State<PedestrainReport> {
  @override
  Widget build(BuildContext cntext) {
    return MaterialApp(
      title: 'See Pedestrain Report',
      ...
      home: Scaffold(
        appBar: AppBar(
          ...
          title: Text('See Pedestrain Report'),
        ),
        body: BodyWidget(),
      ),
    );
...
class BodyWidgetState extends State<BodyWidget> {
  ...
  _makeGetRequest(String id) async {
    final url = Uri.parse('http://192.168.43.129:2700/api/auth/' + id);
    Response response = await http.get(url);
    setState(() {
      final jsonData = jsonDecode(response.body);
      serverResponse1 = jsonData['data'][0]['report_id'];
      serverResponse2 = jsonData['data'][0]['pedestrain_name'];
      serverResponse3 = jsonData['data'][0]['pedestrain_gender'];
      serverResponse4 = jsonData['data'][0]['pedestrain_age'];
